...
#                 self.receive_queue.put(r_header)
#
#                 r_message = self.receive_queue_manager(client_socket, True, FlagEnum.ACK_KA.value)
...
#             client_socket.bind(("127.0.0.1", 62933))
...
#             print(f"{cp.BLUE}- Sending INF{cp.RESET}")
#
#             r_data = client_socket.recv(self.buff_size)
#             self.receive_queue.put(r_data)
#
#             # receiving response from the server
#             r_data = self.receive_queue_manager(client_socket, False, FlagEnum.ACK.value)
#
...
#                     print(f"{cp.BLUE}- Sending DATA{cp.RESET}")
#
...
#                         self.receive_queue.put(r_data)
#
#                         r_data = self.receive_queue_manager(client_socket, False)
#                         r_flag = self.menu.get_flag(r_data)
...
#     #             return r_data
#
#     def receive_queue_manager(self, client_socket: socket, ka_flag: bool, flag: int = 3) -> bytes:
#         is_send = False
#
#         try:
#             while True:
#                 try:
#                     if is_send:
#                         r_header, self.client_address = client_socket.recvfrom(self.buff_size)
#                         self.receive_queue.put(r_header)
#                         is_send = False
#
...
#                     client_socket.sendto(self.initialize_message(r_flag, r_frag_order, r_data, r_crc), self.client_address)
#                     is_send = True
#
#                 except queue.Empty:
